...
@protocol TVOIncomingCallDelegate;
@protocol TVOOutgoingCallDelegate;
 ...
 * @param delegate A `<TVOOutgoingCallDelegate>` to receive call state updates.
 *
 * @return A `<TVOOutgoingCall>` object.
 *
 * @see TVOOutgoingCall
 * @see TVOOutgoingCallDelegate
 */
- (nullable TVOOutgoingCall *)call:(nonnull NSString *)accessToken
                            params:(nullable NSDictionary <NSString *, NSString *> *)twiMLParams
                          delegate:(nullable id<TVOOutgoingCallDelegate>)delegate;
